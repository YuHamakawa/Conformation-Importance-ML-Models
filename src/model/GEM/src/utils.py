...
from pahelix.datasets.aptc_dataset import *
from pahelix.datasets.mp_dataset import *
from pahelix.datasets.pqc_dataset import *
from pahelix.datasets.qm9_gdb_dataset import *
from pahelix.utils.splitters import (CidSplitter, IndexSplitter,
                                     RandomScaffoldSplitter, RandomSplitter,
                                     ScaffoldSplitter)
from sklearn.metrics import roc_auc_score

# from paddle import fluid # use base instead of fluid
...
def get_downstream_task_names(dataset_name, data_path):
    ...
    elif dataset_name == 'pqc':
        return get_default_pqc_task_names()
    elif dataset_name == 'aptc1' or dataset_name == 'aptc2':
        return get_default_aptc_task_names()
    elif dataset_name == 'mp':
        return get_default_mp_task_names()
    else:
...
def get_dataset(dataset_name, data_path, task_names, test_mode=False):
    ...
    if dataset_name == 'bace':
        ...
        dataset = load_pqc_dataset(data_path, task_names, test_mode)
    elif dataset_name == 'aptc1' or dataset_name == 'aptc2':
        dataset = load_aptc_dataset(data_path, task_names, test_mode)
    elif dataset_name == 'mp':
        dataset = load_mp_dataset(data_path, task_names, test_mode)
    else:
        raise ValueError('%s not supported' % dataset_name)
...
def get_dataset_stat(dataset_name, data_path, task_names, test_mode=False):
    """tbd"""
    if dataset_name == 'esol':
        ...
        return get_pqc_stat(data_path, task_names, test_mode)
    elif dataset_name == 'aptc1' or dataset_name == 'aptc2':
        return get_aptc_stat(data_path, task_names, test_mode)
    elif dataset_name == 'mp':
        return get_mp_stat(data_path, task_names, test_mode)
    else:
