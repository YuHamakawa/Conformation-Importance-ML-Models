...
class DownstreamTransformFn_PQC(object):
    ...
    def __call__(self, raw_data):
        ...
        if mol is None:
            return None
        data = mol_to_geognn_graph_data_PQC(mol)
        if not self.is_inference:
            data['label'] = raw_data['label'].reshape([-1])
        data['cid'] = int(mol.GetProp('_Name').split('_')[0])
        return data


class DownstreamTransformFn_APTC(object):
    ...
    def __call__(self, raw_data):
        ...
        if not self.is_inference:
            data['label'] = raw_data['label'].reshape([-1])
        data['cid'] = (mol.GetProp('_Name').split('_')[0] + '_' +
                       mol.GetProp('_Name').split('_')[1])
        return data
