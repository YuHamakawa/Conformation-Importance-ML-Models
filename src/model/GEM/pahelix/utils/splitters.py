...
class ScaffoldSplitter(Splitter):
    ...
    def split(self, dataset, frac_train=None, frac_valid=None, frac_test=None):
        ...
        all_scaffolds = {}
        for i in range(N):
            scaffold = generate_scaffold(dataset[i]['smiles'],
        ...
        all_scaffold_sets = [
            scaffold_set
            for (scaffold,
                 scaffold_set) in sorted(all_scaffolds.items(),
...
class RandomScaffoldSplitter(Splitter):
    ...
    def split(self,
        ...
        for ind in range(N):
            scaffold = generate_scaffold(dataset[ind]['smiles'],
                                         include_chirality=True)
...
class CidSplitter(Splitter):

    def __init__(self):
        super(CidSplitter, self).__init__()

    def split(self, dataset, train_cid_path, test_cid_path):
        train_cid_set = set(np.load(train_cid_path))
        test_cid_set = set(np.load(test_cid_path))
        ...
        train_val_idx = [
            index for index, cid in enumerate(cid_list) if cid in train_cid_set
        ]
        test_idx = [
            index for index, cid in enumerate(cid_list) if cid in test_cid_set
        ]

        # split train validation into 8:2 ratio
        np.random.seed(42)
        train_val_idx = np.random.permutation(train_val_idx)
        split_point = int(len(train_val_idx) * 0.8)
        train_idx = train_val_idx[:split_point].tolist()
        valid_idx = train_val_idx[split_point:].tolist()
